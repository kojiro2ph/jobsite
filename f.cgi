...
<html>
    ...
    <body>
        <form name="default_form" enctype="multipart/form-data" method
+="post">
            <p>
...
<script language="Javascript"> 
    setInterval("check_status(['check_upload__1', 'rand_string', 'uplo
+adedfile'], ['statusbar']);",'1000')
</script> 
            <div id="statusbar">
            </div>
    </body>
...
my $outfile = $Upload_Dir . '/' . $q->param('rand_string') . '-' . $q-
+>param('uploadedfile');
my $p = <<EOF
...
sub upload_that_file { 
    ...
    my $outfile = $Upload_Dir . '/' . $q->param('rand_string') . '-' .
+ $q->param('uploadedfile');
...
+r die $!;
   ...
   my $big = $s * 5; 
    ...
    my $r = '<h1>' . $s . '%</h1>'; 
       $r .= '<div style="width:' . $big . 'px;height:25px;background-
+color:#6f0;float:left"></div>'; 
       $r .= '<div style="width:' . $small . 'px;height:25px;backgroun
+d-color:f33;float:left"></div>';
    return $r; 
